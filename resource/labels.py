...
from all_functions import open_json, put_id_to_tags, write_json # frequently used functions in the project
...
for data in tqdm(data_dict):
    ...
    for item in items:
        tags = item['Tags']
        image_to_label[item['SKU_ID']] = []
        if '98' in tags.keys(): # women's clothes
            label_list = []
            for t in tags['98']:
                label_list.append(t['label_name'])
            labels_type.extend(label_list)
            if len(image_to_label[item['SKU_ID']])==0:
                image_to_label[item['SKU_ID']] = label_list
...
for im in image_to_label:
    image_to_label[im] = labels_dict[image_to_label[im][0]]
